const mapOrderItemsId = (ids) => ids
   .reduce((list, id) => `${list}
   <edis:OrderItemID Owner="AF">${id}</edis:OrderItemID>`
   , '');

const mapPassengerList = (passengers) => passengers
   .reduce((list, passenger) => `${list}
   <edis:Passenger PassengerID="${passenger}"/>`, '');


const fulfillOrderTemplate = (data) => `<soapenv:Envelope xmlns:edis="http://www.iata.org/IATA/EDIST/2017.1" xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/">
<soapenv:Header>
